...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data) {
  ...
  return "  <div class=\"column "
    ...
    + "\">\r\n    <div class=\"search\">\r\n      <input type=\"text\" id=\"search\" placeholder=\"Начните вводить имя друга\">\r\n    </div>\r\n    <div class=\"friends-list\">\r\n      <div class=\"friends-title\">"
...
var _logger = __webpack_require__(93);
...
module.exports = {
    login: function login(appID, permissions) {
    ...
    callApi: function callApi(method, options) {
        !options.v && (options.v = '5.64');
...
var friendTemplate = __webpack_require__(81);
var DragManager = __webpack_require__(46);

module.exports = new function () {
    var allFriends = void 0;
    var friendsContainer = void 0;
    var selectedContainer = void 0;
    ...
    function renderFriends(friends, container) {
        var htmlContainer = container.querySelector('.friends-container');
    ...
    function searchFriendsInArray(friends, filter) {
        ...
        if (filter !== '' && friends.length) {
            filteredUsers = friends.filter(function (user) {
                if (user.first_name.toLowerCase().indexOf(filter) !== -1 || user.last_name.toLowerCase().indexOf(filter) !== -1) {
                    return user;
                }
            });
        } else {
    ...
    function searchFriendsInDOM(container) {
        var friends = (0, _from2.default)(container.querySelectorAll('.friend'));
        var filter = container.querySelector('input').value;

        friends.forEach(function (item) {
            var name = item.querySelector('.friend__name').innerText;
    ...
    function moveFriend(id, container) {
        var friend = document.querySelector('[data-id="' + id + '"]');
        var htmlContainer = container.querySelector('.friends-container');
        var cloneFriend = friend.cloneNode(true);
    ...
    function saveFriends(friends) {
        ...
        storage.selectedFriends = (0, _stringify2.default)(friends);
    }
    ...
    this.init = function (friends, container) {
        ...
        if (localStorage.hasOwnProperty('selectedFriends')) {
            var answer = confirm('Загрузить последнее сохранение?');
        ...
        container.addEventListener('keyup', function (e) {
            ...
            if (e.target.tagName === 'INPUT') {
                var _container = target.parentNode.parentNode;

                searchFriendsInDOM(_container);
            }
        });
        ...
        container.addEventListener('click', function (e) {
            ...
            if (target.tagName === 'BUTTON') {
                var friendID = e.target.parentNode.dataset.id;
                var where = void 0;
        ...
        saveButton.addEventListener('click', function () {
            return saveFriends(allFriends);
        });

        DragManager.onDragEnd = function (dragObject, dropElem) {
            var friendID = dragObject.elem.dataset.id;
            var container = dropElem.parentNode.parentNode;
...
module.exports = new function () {
    ...
    function setPlacemarks(data) {
        data.forEach(function (friend) {
            ...
            var location = friend.country.title + ' ' + friend.city.title;
    ...
    function createLayout() {
        ...
        return ymaps.templateLayoutFactory.createClass(['<h2 class="title">{{geoObject.properties.placemarkLocation}}</h2>',
        // Выводим в цикле список всех геообъектов.
        // '{% for geoObject in properties.geoObjects %}',
        '<div class="friend" data-id="{{ geoObject.properties.placemarkId }}">', '<div class="friend__avatar">', '<img src="{{ geoObject.properties.placemarkPhoto }}" class="friend__img"></div>', '<a href="https://vk.com/id{{geoObject.properties.placemarkId}}" class="friend__name" target="_blank">{{ geoObject.properties.placemarkName }}</a>', '</div>'].join(''));
    }
...
module.exports = new function () {
    ...
    function onMouseMove(e) {
        ...
        if (!dragObject.avatar) {
            ...
            var avatarCoords = getCoords(dragObject.elem);
        ...
        var coordX = e.pageX - dragObject.shiftX;
        var coordY = e.pageY - dragObject.shiftY;

        dragObject.avatar.style.left = coordX + 'px';
        dragObject.avatar.style.top = coordY + 'px';

        // console.log('координаты дива в момент перемещения %s %s',coordX, coordY);
        // console.log('координаты курсора в момент перемещения %s %s', e.pageX, e.pageY);

        // обновляем координаты курсора при движении
        // dragObject.downX = e.pageX;
        // dragObject.downY = e.pageY;
...
var friendTemplate = __webpack_require__(25);
var VKAPI = __webpack_require__(43);
var FriendOnMap = __webpack_require__(45);
var FriendFilter = __webpack_require__(44);
...
window.onload = function () {
    ...
    auth.addEventListener('click', function () {
        //todo авторизация по кнопке (раскомментировать)
        VKAPI.login(VKID, VKPERMISSIONS).then(function (response) {
            return new _promise2.default(function (resolve) {
                return ymaps.ready(resolve);
            });
        }).then(function () {
            return VKAPI.getFriends();
    ...
    mapButton.addEventListener('click', function () {
        if (!flag) {
            ...
            flag = true;
            _this.innerText = 'Вернуться к спискам';
        } else {
            flipper.style.transform = 'rotateY(0)';
            flag = false;
            _this.innerText = 'Показать на карте';
        }
    });
};
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data) {
  ...
  return "  <div class=\"friend\" data-id=\""
    ...
    + alias4(((helper = (helper = helpers.id || (depth0 != null ? depth0.id : depth0)) != null ? helper : alias2),(typeof helper === alias3 ? helper.call(alias1,{"name":"id","hash":{},"data":data}) : helper)))
    + "\" class=\"friend__name\" target=\"_blank\">\r\n      "
...
var _handlebarsNoConflict = __webpack_require__(94);
...
var _decoratorsInline = __webpack_require__(84);
